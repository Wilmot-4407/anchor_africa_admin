...
import { X, UploadCloud, Sparkles, Loader2, Check, ChevronRight, ChevronLeft } from "lucide-react";
...
const STEPS = ["Identity", "Content", "Details"];
    ...
    <div className="flex items-start px-6 py-4 border-b border-slate-100">
      {steps.map((label, i) => (
        <Fragment key={i}>
          ...
          {i < steps.length - 1 && (
            <div className={`flex-1 h-px mx-3 mt-3.5 transition-all ${i < current ? "bg-primary" : "bg-slate-200"}`} />
          )}
        </Fragment>
      ))}
    </div>
...
function IconSuggestButton({
  title, shortDescription, fullDescription, currentIcon, onSelect, compact = false,
}: {
  title: string; shortDescription: string; fullDescription: string;
  currentIcon: string; onSelect: (n: string) => void; compact?: boolean;
}) {
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      const { data } = await api.post("/ai/suggest-icon", {
        ...
        fullDescription: fullDescription?.trim() || "", icons: LUCIDE_ICON_NAMES,
      });
      ...
      if (err.response?.status === 401) msg = "Please log in again (session expired)";
  ...
  return (
    <>
      <button type="button" onClick={fetchSuggestions} disabled={loading} title="AI icon suggestions"
        className={compact
      ...
      {open && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
          <div className="bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden w-80 max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-violet-50 to-purple-50 border-b border-slate-100 flex-shrink-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-slate-700">AI Icon Suggestions</span>
              </div>
              <button type="button" onClick={() => setOpen(false)} className="p-1 rounded-md text-slate-400 hover:text-slate-600 hover:bg-white/60 transition-colors">
            ...
            {(title || shortDescription) && (
              <div className="px-4 py-2 bg-slate-50 border-b border-slate-100 flex-shrink-0">
                <p className="text-[11px] text-slate-400 truncate">
                  <span className="font-medium text-slate-500">{title || shortDescription}</span>
                </p>
              </div>
            )}
            <div className="p-3 overflow-y-auto">
              ...
              {error && !loading && <div className="py-4 text-center"><p className="text-xs text-red-500">{error}</p></div>}
              {!loading && !error && suggestions.length > 0 && (
                <>
                  <p className="text-xs text-slate-400 mb-3">Click an icon to use it · sorted by relevance</p>
                    ...
                    {suggestions.map((s) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[8px] text-slate-400 leading-tight text-center group-hover:text-violet-400 transition-colors">{s.score}/10</span>
                        </button>
                      );
                    })}
                  </div>
                  <p className="mt-3 text-[10px] text-slate-300 text-center">Stored as Lucide icon name</p>
                </>
              )}
            </div>
          </div>
...
export function ServiceForm({ service, onClose, onSuccess }: ServiceFormProps) {
  ...
  const validateStep = (s: number): boolean => {
    if (s === 0) {
      ...
      if (!formData.category.trim()) { setFormError("Category is required"); return false; }
    }
    if (s === 1 && !formData.shortDescription.trim()) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fd.append("features", formData.features.split(",").map((f) => f.trim()).filter(Boolean).join(","));
      fd.append("benefits", formData.benefits.split(",").map((b) => b.trim()).filter(Boolean).join(","));
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[92vh] flex flex-col border border-slate-200/60">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 flex-shrink-0">
          <div>
            <h2 className="text-lg font-bold text-slate-800">{isEditing ? "Edit Service" : "Create Service"}</h2>
        ...
        {/* Scrollable body */}
        <form id="service-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          {(formError || error) && (
            <div className="px-4 py-3 bg-red-50 border border-red-200 rounded-xl">
              <p className="text-sm text-red-600 font-medium">{formError || error}</p>
            </div>
          )}
          ...
          {step === 0 && (
            <section>
              <SectionHeading>Service Identity</SectionHeading>
              <div className="space-y-4">
                <div>
                  <Label>Category Image</Label>
                  <input ref={fileInputRef} type="file" id="service-image-input" accept="image/*" onChange={handleImageChange} disabled={isLoading} className="hidden" />
                  <label htmlFor="service-image-input" className="block w-full cursor-pointer">
                ...
                <div>
                  ...
                  <input type="text" name="title" value={formData.title} onChange={handleChange} disabled={isLoading} className={inputCls} placeholder="Clinical Services" />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label required>Type</Label>
                    <select name="type" value={formData.type} onChange={handleChange} disabled={isLoading} className={inputCls}>
                  ...
                  <div>
                    ...
                    <input type="text" name="category" value={formData.category} onChange={handleChange} disabled={isLoading} className={inputCls} placeholder="Mental Health" />
                  </div>
                </div>
          ...
          {step === 1 && (
            <section>
              <SectionHeading>Descriptions &amp; Icon</SectionHeading>
              <div className="space-y-4">
                <div>
                  <Label required>Short Description (card text)</Label>
                  <textarea name="shortDescription" value={formData.shortDescription} onChange={handleChange} disabled={isLoading} rows={3} className={`${inputCls} resize-none`} placeholder="Brief overview shown on cards..." />
                </div>

                <div>
                  <Label>Full Description (detail page)</Label>
                  <textarea name="fullDescription" value={formData.fullDescription} onChange={handleChange} disabled={isLoading} rows={7} className={`${inputCls} resize-y font-mono text-xs`} placeholder="Rich HTML or plain-text content..." />
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <Label>Icon</Label>
                    <IconSuggestButton
                      title={formData.title} shortDescription={formData.shortDescription}
                      fullDescription={formData.fullDescription} currentIcon={formData.icon}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input type="text" name="icon" value={formData.icon} onChange={handleChange} disabled={isLoading} className={inputCls} placeholder="Heart · Brain · Stethoscope (Lucide name)" />
                  </div>
                </div>
              </div>
            </section>
          ...
          {step === 2 && (
            <section>
              <SectionHeading>Features &amp; Publishing</SectionHeading>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label>Features (comma-separated)</Label>
                    <input type="text" name="features" value={formData.features} onChange={handleChange} disabled={isLoading} className={inputCls} placeholder="Feature 1, Feature 2" />
                  </div>
                  <div>
                    <Label>Benefits (comma-separated)</Label>
                    <input type="text" name="benefits" value={formData.benefits} onChange={handleChange} disabled={isLoading} className={inputCls} placeholder="Benefit 1, Benefit 2" />
                  </div>
                </div>

                <div className="flex items-center gap-3 pt-1">
                  <input type="checkbox" name="isPublished" id="isPublished" checked={formData.isPublished} onChange={handleChange} disabled={isLoading} className="w-4 h-4 accent-primary" />
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl flex-shrink-0">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <button type="submit" form="service-form" disabled={isLoading} className="ml-auto flex items-center gap-2 px-5 py-2 bg-primary hover:bg-primary-dark text-white text-sm font-bold rounded-xl transition disabled:opacity-50 shadow-sm">
