...
}: {icon: React.ReactNode;iconBg: string;iconColor: string;label: string;value: string;change?: string;changeUp?: boolean;}) {
  return (
    <motion.div
      whileHover={{
        y: -2
...
function AppointmentRow({
...
}: {time: string;client: string;service: string;clinician: string;status: 'confirmed' | 'pending' | 'completed' | 'cancelled';}) {
  const statusStyles = {
  ...
  return (
    <div className="flex items-center gap-4 py-3 border-b border-border/50 last:border-0">
      <span className="text-sm font-medium text-navy-950 w-20 flex-shrink-0">
        {time}
      </span>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-navy-950 truncate">{client}</p>
...
function QuickAction({
  ...
  return (
    <button
      onClick={onClick}
      className="flex flex-col items-center gap-2 p-4 bg-white rounded-xl border border-border hover:border-accent/30 hover:shadow-sm transition-all text-center group">

      <span className="text-slate-400 group-hover:text-accent transition-colors">
        {icon}
      </span>
      <span className="text-xs font-medium text-slate-600 group-hover:text-navy-950 transition-colors">
        {label}
      </span>
    </button>);
...
function ActivityItem({
  ...
  return (
    <div className="flex items-start gap-3">
      <div className="w-8 h-8 rounded-full bg-navy-900 flex items-center justify-center text-white text-[10px] font-semibold flex-shrink-0 mt-0.5">
        {avatar}
      </div>
      <div className="flex-1 min-w-0">
